...
$intake_sources = [];
$stmt_src = $pdo->query("SELECT iid, sname FROM intake_source");
$intake_sources = $stmt_src->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($message)) {
        ...
        try {
            ...
            if ($stmt->execute()) {
                $pdo->prepare("INSERT INTO user_activity_log (uid, actiontype, targettable, targetid, details) VALUES (:uid,'Update','animals',:targetid,'Updated animal record')")->execute([':uid'=>$_SESSION['uid'],':targetid'=>$animal_id]);
                $message = 'success';
            }
...
<div class="main">
  <header class="topbar">
    ...
    <span class="topbar-title"><span data-i18n="edit_animal">Edit Animal</span>: <?= htmlspecialchars($animal['name']) ?></span>
    <div class="tb-right">
...
<script>
const tr={en:{shelter_sys:"Shelter System",overview:"Overview",animals_sec:"Animals",people_sec:"People",system_sec:"System",dashboard:"Dashboard",animals:"Animals",add_animal:"Add Animal",adopters:"Adopters",add_adopter:"Add Adopter",reports:"Reports",admin_ctrl:"Admin Controls",sign_out:"Sign Out",edit_animal:"Edit Animal",back_animals:"Back to Animals",update_ok:"Animal record updated successfully!",error_msg:"An error occurred. Please try again.",animal_name:"Name",species:"Species",breed:"Breed",gender:"Gender",unknown:"Unknown",male:"Male",female:"Female",age_yrs:"Age (Years)",status:"Status",available:"Available",pending:"Pending",medical_care:"Medical Care",adopted:"Adopted",intake_src:"Intake Source",update_photo:"Update Photo (leave blank to keep current)",update_btn:"Update Animal"},ku:{shelter_sys:"سیستەمی پەناگا",overview:"پوختە",animals_sec:"ئاژەڵەکان",people_sec:"کەسەکان",system_sec:"سیستەم",dashboard:"داشبۆرد",animals:"ئاژەڵەکان",add_animal:"زیادکردنی ئاژەڵ",adopters:"خاوەن نوێکان",add_adopter:"زیادکردنی وەرگر",reports:"ڕاپۆرتەکان",admin_ctrl:"بەڕێوەبردن",sign_out:"دەرچوون",edit_animal:"دەستکاریکردنی ئاژەڵ",back_animals:"گەڕانەوە بۆ ئاژەڵەکان",update_ok:"تۆماری ئاژەڵ بە سەرکەوتوویی نوێکرایەوە!",error_msg:"هەڵەیەک ڕوویدا. تکایە دووبارە هەوڵبدەرەوە.",animal_name:"ناو",species:"جۆر",breed:"نەژاد",gender:"ڕەگەز",unknown:"نەزانراو",male:"نێر",female:"مێ",age_yrs:"تەمەن (ساڵ)",status:"دەربارە",available:"بەردەستە",pending:"چاوەڕوان",medical_care:"چارەسەری پزیشکی",adopted:"قبووڵکرا",intake_src:"سەرچاوەی وەرگرتن",update_photo:"نوێکردنەوەی وێنە (بۆ هێشتنەوەی ئەمەی هەیە بەتاڵ بهێڵەوە)",update_btn:"نوێکردنەوەی ئاژەڵ"}};
