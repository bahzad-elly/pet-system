...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $stmt->bindParam(':fname',$fname);$stmt->bindParam(':lname',$lname);
        $stmt->bindParam(':dob',$dob);$stmt->bindParam(':phone',$phone);
        $stmt->bindParam(':address',$address);$stmt->bindParam(':preference',$preference);
        if ($stmt->execute()) {
            $adopter_id = $pdo->lastInsertId();
            $pdo->prepare("INSERT INTO user_activity_log (uid, actiontype, targettable, targetid, details) VALUES (:uid,'Create','adopters',:targetid,'Registered new adopter: $fname $lname')")->execute([':uid'=>$_SESSION['uid'],':targetid'=>$adopter_id]);
            $message = 'success';
        }
...
.card{background:var(--card);border:1px solid var(--border);border-radius:14px;padding:24px;animation:fadeUp .5s ease both;max-width:680px;}
...
.btn-b{background:var(--b);color:#fff;} .btn-b:hover{background:#2563eb;transform:translateY(-1px);}
...
<aside class="sbar" id="sbar">
  ...
  <nav class="sbar-nav">
    ...
    <a href="register_adopter.php" class="ni on"><i class="fas fa-user-plus"></i><span class="nl" data-i18n="add_adopter">Add Adopter</span></a>
...
<div class="main">
  <header class="topbar">
    ...
    <span class="topbar-title" data-i18n="reg_adopter">Register Adopter</span>
    <div class="tb-right">
...
<script>
const tr={en:{shelter_sys:"Shelter System",overview:"Overview",animals_sec:"Animals",people_sec:"People",system_sec:"System",dashboard:"Dashboard",animals:"Animals",add_animal:"Add Animal",adopters:"Adopters",add_adopter:"Add Adopter",reports:"Reports",admin_ctrl:"Admin Controls",sign_out:"Sign Out",reg_adopter:"Register Adopter",reg_new_adopter:"Register New Adopter",back_adopters:"Back to Adopters",success_msg:"Adopter registered successfully!",error_msg:"An error occurred. Please try again.",fname:"First Name",lname:"Last Name",dob:"Date of Birth",phone:"Phone Number",address:"Address",preference:"Adoption Preferences",reg_btn:"Register Adopter"},ku:{shelter_sys:"سیستەمی پەناگا",overview:"پوختە",animals_sec:"ئاژەڵەکان",people_sec:"کەسەکان",system_sec:"سیستەم",dashboard:"داشبۆرد",animals:"ئاژەڵەکان",add_animal:"زیادکردنی ئاژەڵ",adopters:"خاوەن نوێکان",add_adopter:"زیادکردنی وەرگر",reports:"ڕاپۆرتەکان",admin_ctrl:"بەڕێوەبردن",sign_out:"دەرچوون",reg_adopter:"تۆمارکردنی وەرگر",reg_new_adopter:"تۆمارکردنی وەرگری نوێ",back_adopters:"گەڕانەوە بۆ وەرگرەکان",success_msg:"وەرگر بە سەرکەوتوویی تۆمارکرا!",error_msg:"هەڵەیەک ڕوویدا. تکایە دووبارە هەوڵبدەرەوە.",fname:"ناوی یەکەم",lname:"ناوی کۆتایی",dob:"بەرواری لەدایکبوون",phone:"ژمارەی تەلەفۆن",address:"ناونیشان",preference:"ئارەزووی قبووڵکردن",reg_btn:"تۆمارکردنی وەرگر"}};
